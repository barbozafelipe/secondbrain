...
- **27/07/2026 (Seg) — drill de governança + Storage Gateway (feito via chat, sem motivação no início do dia — sessão aconteceu mesmo assim, formato de pergunta única e zero decisão funcionou).** 4/5 acertos. **Governança (Config vs. Trusted Advisor vs. Service Catalog vs. CloudFormation) finalmente estabilizou** — acertou nos dois ângulos testados (evento em tempo real vs. visão geral; autosserviço em catálogo vs. template puro). Considerar esse bloco fechado, salvo erro futuro. **Storage Gateway confirmado como gap real**: errou um cenário clássico de Tape Gateway (confundiu com AWS Backup) — regra fixada: menção a "fita"/"tape" no enunciado = Tape Gateway, nunca AWS Backup (que é backup nativo de recursos AWS, não substituto de biblioteca de fitas on-premises).
