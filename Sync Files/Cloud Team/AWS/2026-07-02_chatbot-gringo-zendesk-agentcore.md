...
- **✅ Mecânica completa das esteiras documentada (22/07/2026)** — ver nota [[Esteira AWS - mecanica das pipelines (Lambda, SQS-Dynamo, Terraform)]]: templates reais, estrutura de arquivo/pasta esperada, como o ambiente é resolvido, e o modelo de "3 peças" (biblioteca remota + repo do projeto + environments).
- **🔴 BLOQUEADOR CONFIRMADO — a esteira de Lambda não suporta Python.** Confirmado por **leitura do código** dos templates (não da doc): `install-dependencies` roda `npm install` fixo; `unittest` roda `npm run test`; o build roda `node awsDeploy.js`; e o passo "Zip Artifacts" é `if NJS … elif NTS …` **sem `else`** — repo que não seja NJS/NTS não gera zip nenhum. PATH do runner só tem node/maven/jdk. As 3 Lambdas do chatbot são Python → **não há caminho de adaptação do nosso lado**, ou o DevOps estende a esteira ou o projeto precisa de outra estratégia de deploy.
- **🟡 CORREÇÃO — "S3 é esteira" provavelmente NÃO se aplica ao bucket da Knowledge Base.** O `pipeline-s3.yml` faz `aws s3 sync`/`cp` de `.js`/`.css`/`.html` + invalidação de CloudFront — é **publicação de site estático em bucket existente**, não criação de bucket. O bucket `ggo-chatbot-documents-qa` não é site estático, então o stack `knowledge-base-serverless` **pode não estar bloqueado de verdade**. Confirmar com Isaelin, mas a premissa que travou o stack parece ter sido leitura ampla demais da palavra "S3".
- **🔴 BLOQUEADOR — não existe repositório de projeto.** Descoberto ao investigar por que o Francisco recebeu os templates e não sabia onde executar: os templates são uma **biblioteca chamada remotamente** (`workflow_call`), não algo que se copia. Falta o repo do projeto (padrão `NJS-*`/`NTS-*`) com um `main.yml` de ~15 linhas apontando pro template. **Esse único item trava Lambda, SQS, DynamoDB e API Gateway ao mesmo tempo.**
- **Gap secundário:** o repo de referência Terraform (`fleetcorbr-aws-repo-iac`) só cobre contas Corpay/SemParar clássicas (FINTECH/FREEFLOW/INFRA/PARCERIAS/STP/TRIADE) — não tem Zapay/Gringo. Tecnicamente é só configurar outra role via OIDC, mas confirmar com DevOps.
