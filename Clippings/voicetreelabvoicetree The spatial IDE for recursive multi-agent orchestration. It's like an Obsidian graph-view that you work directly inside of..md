---
title: "voicetreelab/voicetree: The spatial IDE for recursive multi-agent orchestration. It's like an Obsidian graph-view that you work directly inside of."
source: "https://github.com/voicetreelab/voicetree"
author:
published:
created: 2026-06-25
description: "The spatial IDE for recursive multi-agent orchestration. It's like an Obsidian graph-view that you work directly inside of.  - voicetreelab/voicetree: The spatial IDE for recursive multi-agent orchestration. It's like an Obsidian graph-view that you work directly inside of."
tags:
  - "clippings"
---
...
[![Voicetree Demo](https://github.com/voicetreelab/voicetree/raw/main/meta/core_loop_only_agents.gif)](https://github.com/voicetreelab/voicetree/blob/main/meta/core_loop_only_agents.gif)
...
- agent orchestration tools, such as
	- run(Agent,node) // default behaviour is collect nearby nodes as context and send\_message(agent, local\_graph(node)) it into the spawned agent
...
Windows: [https://github.com/voicetreelab/voicetree/releases/latest/download/voicetree.exe](https://github.com/voicetreelab/voicetree/releases/latest/download/voicetree.exe)
...
See [`docker/README.md`](https://github.com/voicetreelab/voicetree/blob/main/docker/README.md) for details. amd64 only for now.

**Run VoiceTree in the browser (no Electron):** a monorepo-only dev command (`@voicetree/cli` isn't published yet) — see [docs/browser-mode.md](https://github.com/voicetreelab/voicetree/blob/main/docs/browser-mode.md).
...
You can spawn coding agents on a node, the contents of that node will become the agents task, and it will also be given all context within an adjustable distance around them, and can semantic search against local embeddings. This means agents see what you see. You share the same memory, the same second brain. The graph structure allows for context retrieval to be targeted to only what is most relevant rather than dumping entire conversation history - avoiding the 30-60% performance degradation from context rot [^1].
...
BSL 1.1, converts to Apache 2.0 after 4 years. See [LICENSE](https://github.com/voicetreelab/voicetree/blob/main/LICENSE).
...
[^1]: Chroma Research, "Context Rot: How Increasing Input Tokens Impacts LLM Performance" (July 2025). 30-60% performance gaps between focused (~300 token) and full (~113k token) prompts. [https://research.trychroma.com/context-rot](https://research.trychroma.com/context-rot)