module "container_app_env" {
  ...
  container_app_env_name     = "stp-dig-cae-aihub-nprd"
...
module "container_app_mcp" {
  ...
  container_app_name   = "stp-dig-acapps-gptappmcp-nprd"
  container_app_env_id = module.container_app_env.id
  registry_endpoint    = module.acr.acr_endpoint

  template = {
    name         = "stp-dig-acapps-gptappmcp-nprd"
    image        = "stpdigacraihubnprd.azurecr.io/stp-ms-ai/nstaz-ai-gptappmcp-dev:7"
    cpu          = 0.5
    memory       = "1Gi"
    min_replicas = 1
    max_replicas = 3
